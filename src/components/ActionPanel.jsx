...
import { ngcp_line6, ngcp_line7, ngcp_pole6, ngcp_pole7, ngcp_working_area6 } from '../layers';

function ActionPanel() {
  const [activeWidget, setActiveWidget] = useState(null);
  const [nextWidget, setNextWidget] = useState(null);
  const arcgisScene = document.querySelector('arcgis-scene');
  const directLineMeasure = document.querySelector('arcgis-direct-line-measurement-3d');
  ...
  useEffect(() => {
    if (activeWidget) {
      ...
      directLineMeasure ? directLineMeasure.clear() : console.log('Line measure is cleared');
      areaMeasure ? areaMeasure.clear() : console.log('Area measure is cleared.');
    }
          ...
          <arcgis-layer-list
            ...
            onarcgisTriggerAction={(event) => {
              ...
              if (id === 'full-extent-ngcpwa6') {
                arcgisScene?.goTo(ngcp_working_area6.fullExtent).catch((error) => {
        ...
        <CalcitePanel heading="Description" data-panel-id="information" hidden>
          {nextWidget === 'information' ? (
            <div style={{ paddingLeft: '20px' }}>
              ...
              <div style={{ paddingLeft: '20px' }}>
                <li>
                  The source of data: <b>Master List tables</b> provided by the RAP Team.
                </li>
              </div>
            </div>
          ) : (
